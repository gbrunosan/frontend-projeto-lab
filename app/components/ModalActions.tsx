...
const ModalActions: React.FC<ModalActionsProps> = ({
  ...
  return (
    <div
      className={`fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex justify-center items-center z-10 modal-container px-4 transition-opacity duration-150 ${
        isVisible ? "opacity-100" : "opacity-0 pointer-events-none"
      }`}
      onClick={handleOutsideClick}
    >
      <div
        className={`bg-tertiary p-6 h-[230px] rounded-lg shadow-lg w-full max-w-[550px] relative flex flex-col justify-around transform transition-all duration-150 ease-out ${
